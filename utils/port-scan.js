...
const scanPort = async (host, port, timeout) => {
  ...
  ;(function(host, port, timeout) {
    ...
    //==> destroy all sockets that do not connect after a certain amount of time. @ param : timeout
    ...
    socket.connect(port, host, () => {
      ...
      //  we don't destroy the socket after the connection becuase we want to listen to data event
      //==> the socket will self-destruct in 2 secs because of the @param : timeout
    })
